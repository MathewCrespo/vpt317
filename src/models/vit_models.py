...
class ViTGAN(nn.Module):
    """Borrowing core idea of GAN to ViT prompt"""
    ...
    def build_backbone(self, prompt_cfg, cfg, adapter_cfg, load_pretrain, vis):
        ...
        self.enc_freeze, self.feat_dim = build_vit_sup_models(
            cfg.DATA.FEATURE, cfg.DATA.CROPSIZE, None, cfg.MODEL.MODEL_ROOT, None, load_pretrain, vis
        )

        # encoder_freeze is used for source dataset, and should be totally frozen
        for k, p in self.enc_freeze.named_parameters():
    ...
    def setup_head(self, cfg):
        ...
        '''
        # The discriminator has the same structure except for the last classification head
    ...
    def forward_encoder(self, x, source, return_feature=False):
        # for gan output
        if self.froze_enc and self.enc.training:
            self.enc.eval()
